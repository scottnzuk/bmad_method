...
# Load core helpers module securely via direct file read and eval
_core_path = str(_PLUGIN_ROOT / "helpers" / "bmad_status_core.py")
with open(_core_path, "r", encoding="utf-8") as f:
    _core_code = f.read()
# Create isolated namespace for core module execution
_core_ns = {"__name__": "bmad_status_core", "__file__": _core_path}
exec(_core_code, _core_ns)
check_agents = _core_ns["check_agents"]
check_modules = _core_ns["check_modules"]
read_state = _core_ns["read_state"]
read_tests = _core_ns["read_tests"]
SKILL_NAMES = _core_ns["SKILL_NAMES"]
...
PHASE_ACTIONS = {
    ...
    "not_initialized": (
        "Initialize BMAD",
        "Create or open a project in Agent Zero, then say: bmad init",
    ),
    "unknown": (
        "Initialize BMAD",
        "Create or open a project in Agent Zero, then say: bmad init",
...
class BmadStatus(ApiHandler):
    async def process(self, input: dict, request: Request) -> dict | Response:
        try:
            ...
            state_file = (
                (project_root / ".a0proj/instructions/02-bmad-state.md")
                if project_root
                else None
            )
            test_dir = (
                (project_root / ".a0proj/_bmad-output/test-artifacts")
                if project_root
                else None
            )
    ...
    def _check_agents(self):
        ...
        broken = [
            {"name": n, "display": AGENT_NAMES.get(n, n), "missing": mf}
    ...
    def _recommend(self, state_file: Path | None, test_dir: Path | None):
        ...
        if skills["broken"]:
            issues.append(
                {
                    "sev": "blocker",
                    "what": str(len(skills["broken"])) + " module(s) missing",
        ...
        if agents["broken"]:
            issues.append(
                {
                    "sev": "warn",
                    "what": str(len(agents["broken"])) + " agent(s) unhealthy",
        ...
        if tests.get("failing", 0) > 0:
            issues.append(
                {
                    "sev": "warn",
                    "what": str(tests["failing"]) + " test(s) failing",
        ...
        if state["issues"]:
            issues.append(
                {
                    "sev": "open",
                    "what": str(len(state["issues"])) + " open ARCH/DEFECT item(s)",
