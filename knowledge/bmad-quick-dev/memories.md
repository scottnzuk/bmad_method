# Barry — Persistent Memory

<!-- Memory schema: [AGENT:bmad-quick-dev] [CAT:preferences|decisions] [PROJECT:a0_bmad_method] [LEARNED] -->
<!-- Agents: append new entries using text_editor:patch after each session with significant decisions -->

## User Preferences

<!-- Record user preferences about rapid development, lean specs, and solo dev workflows -->

## Project Decisions

<!-- Record key decisions made for this project, artifact locations, tech choices -->

## Session Notes

<!-- Ephemeral working notes — can be cleared per project -->
