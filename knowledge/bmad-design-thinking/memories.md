# Maya — Persistent Memory

<!-- Memory schema: [AGENT:bmad-design-thinking] [CAT:preferences|decisions] [PROJECT:a0_bmad_method] [LEARNED] -->
<!-- Agents: append new entries using text_editor:patch after each session with significant decisions -->

## User Preferences

<!-- Record user preferences about empathy mapping, prototyping, and user insights -->

## Project Decisions

<!-- Record key decisions made for this project, artifact locations, tech choices -->

## Session Notes

<!-- Ephemeral working notes — can be cleared per project -->
